...
def index(request,category_id=1):
    ...
    category_noti = get_object_or_404(Catogory,pk=3)
    # category_list = Catogory.objects.all()
    question_list_noti = Question.objects.filter(category=category_noti.id).order_by('-create_date')[:5] 
    ...
    context = {"question_list_one":question_list_one,"question_list_two":question_list_two,"question_list_noti":question_list_noti}    
    # return render(request,'main/main.html',context)
    return render(request,'main/gaon.html',context)
