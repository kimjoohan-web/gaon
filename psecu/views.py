    ...
    if request.method == 'POST':        
        ...
        for key in p_m_c_val_list:            
           ...
           if key.startswith('psecu_'):                 
                ...
                SQL = "insert into psecu_pc_secu_ck_db (p_m_idx_id, p_m_c_val, p_m_c_id_id, p_m_c_indate,p_m_c_wdate,p_m_b) "
                SQL += " values (%s,%s,%s,%s,%s,%s) "
                # return HttpResponse(useridx )
                with connection.cursor() as cursor:
                    cursor.execute(SQL, [p_m_idx_id, p_m_val, useridx, toDay, p_m_c_wdate, p_m_b]) 
        ...
        useridx = request.user.id
        toDay = time.now().strftime('%Y-%m-%d')
        context ={'ps_list_p':ps_list_p,'ps_list_u':ps_list_u,'ps_list_pe':ps_list_pe,'username':username,'toDay':toDay,'useridx':useridx}
